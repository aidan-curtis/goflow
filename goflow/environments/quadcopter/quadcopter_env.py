...
class QuadcopterEnvCfg(DirectRLEnvCfg):
    ...
    num_observations = 12
    dr_ranges = {
        "quadcopter_mass": (0.001, 0.4),
        # "thrust_to_weight": (1.5, 2.5),
    }
    midpoints = {
        "quadcopter_mass": 0.2005,
    }
...
class QuadcopterEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        for key, value in self.get_dr_ranges().items():
            if key == "quadcopter_mass":
                masses = self._robot.root_physx_view.get_masses().clone()

                mass_key = list(self.get_dr_ranges().keys()).index("quadcopter_mass")

                quadcopter_mass_tensor = torch.tensor(context[:, mass_key]).cpu()
                body_ids, _ = self._robot.find_bodies("body")
                masses[cpu_env_ids, body_ids[0]] = quadcopter_mass_tensor

                self._robot.root_physx_view.set_masses(masses, cpu_env_ids)

            elif key == "thrust_to_weight":
                raise NotImplementedError
                ttw_key = list(self.get_dr_ranges().keys()).index("thrust_to_weight")
                self.cfg.thrust_to_weight = context[:, ttw_key]
...
for dr_method in ["NoDR", "FullDR", "LSDR", "ADR", "GOFLOW", "DORAEMON"]:
    print(f"{agents.__name__}:{dr_method}.yaml")
    gym.register(
        id=f"Quadcopter-{dr_method}-v0",
        entry_point="goflow.environments.quadcopter.quadcopter_env:QuadcopterEnv",
