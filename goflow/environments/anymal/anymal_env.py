...
class AnymalCFlatEnvCfg(DirectRLEnvCfg):
    ...
    dr_ranges = {
        # "joint_position_randomization": (-0.125, 0.125),  # Add joint position randomization range
        # "dummy": (0.0, 0.1)
        "base_mass": (0.0, 200.0),
        "LF_HAA_friction": (0, 0.1),
        "RF_HAA_friction": (0, 0.1)
    }
...
class AnymalCEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Sample new commands
        self._commands[env_ids, 0] = 1.0  # Set x-axis velocity to maximum (forward)
        self._commands[env_ids, 1:] = 0.0  # Set other commands (y-axis velocity and yaw rate) to zero
        # Reset robot state
        ...
        if(hasattr(self, "dist")):
            context = self.dist.rsample([cpu_env_ids.shape[0]]).to(self.device)
            ...
            for key, (min_val, max_val) in self.get_dr_ranges().items():
                ...
                if key == "base_mass":
                    ...
                    self._robot.root_physx_view.set_masses(masses, cpu_env_ids)
                elif key == "LF_HAA_friction":
                    friction = self._robot.root_physx_view.get_dof_friction_coefficients().clone()
                    joint_ids, _ = self._robot.find_joints("LF_HAA")
                    friction[cpu_env_ids, joint_ids[0]] = context_tensor
                    self._robot.root_physx_view.set_dof_friction_coefficients(friction, cpu_env_ids)
                elif key == "RF_HAA_friction":
                    friction = self._robot.root_physx_view.get_dof_friction_coefficients().clone()
                    joint_ids, _ = self._robot.find_joints("RF_HAA")
                    friction[cpu_env_ids, joint_ids[0]] = context_tensor
                    self._robot.root_physx_view.set_dof_friction_coefficients(friction, cpu_env_ids)
                # Add more randomizations here as needed
...
for dr_method in ["NoDR", "FullDR", "LSDR", "ADR", "GOFLOW", "DORAEMON"]:
    ...
    gym.register(
        id=f"Anymal-{dr_method}-v0",
        entry_point="goflow.environments.anymal.anymal_env:AnymalCEnv",
