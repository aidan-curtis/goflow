...
from goflow.rl_components.my_running_mean_std import RunningMeanStd, RunningMeanStdObs
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            if (not is_train):
                mu = mu.detach().cpu().to(input_dict['obs'].device)
                logstd = logstd.detach().cpu().to(input_dict['obs'].device)
                value = value.detach().cpu().to(input_dict['obs'].device)
